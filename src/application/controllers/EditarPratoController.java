...
public class EditarPratoController implements Initializable {
	...
	private void checarEntradas() {
		...
		if (Verificacoes.verificaIdPrato(entradaId)) {
			...
			int id2 = Integer.parseInt(entradaId);
				...
				if (!(tamanhoIngredientes.equals(0))) {
					...
					if (Buscas.buscaNomePrato(entradaNome)) {
						...
						if (Verificacoes.verificaConversaoNumeroFloat(entradaPreco)) {
							...
							Prato prato = new Prato(entradaNome, entradaCategoria, entradaDescricao, ingredientesPrato, preco2); 
							prato.setIdPrato(id2);
							GerenciarPrato.editarProduto(prato);
					...
					Alertas.showAlert(null, "Adicione pelo menos um ingrediente para editar um prato", null,
							AlertType.WARNING);
				}
			} else {
			...
			Alertas.showAlert(null, "Id inválido!!", "O id é inválido ou inexistente", AlertType.WARNING);
		}
	}
	...
	public void onBtAdicionar(ActionEvent Event) throws IOException {
		...
		if (ingredientesPrato == null) {
			...
			if ((entradaId != "") && (entradaQuantidade != "")) {
				...
				Alertas.showAlert(null, null, "Preencha todos os campos para cadastrar um ingrediente",
						AlertType.WARNING);
			}

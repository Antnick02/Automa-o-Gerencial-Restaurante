...
public class VendasAbertoController implements Initializable {
	...
	private void checarIdBusca() throws IOException {
		...
		if (Verificacoes.verificaConversaoNumeroInt(idBusca.getText())) {
			...
			if (Verificacoes.verificaIdVenda(idBusca.getText(), GerenciarVenda.getVendasEmAberto())) {
				
				int id = Integer.parseInt(idBusca.getText());
				int posicao;

				posicao = GerenciarVenda.buscaVenda(id, GerenciarVenda.getVendasEmAberto(),
						GerenciarVenda.getVendasEmAberto().size());
				
				Venda venda1 = GerenciarVenda.getVendasEmAberto().get(posicao);
	...
	public void carregandoTableViewVenda() {
		List<Venda> listaVendas = GerenciarVenda.getVendasEmAberto();
	...
	public void onBtExcluir(ActionEvent Event) throws IOException {
		...
		Parent root = FXMLLoader.load(getClass().getResource("/views/ExcluirVendaAbertoView.fxml"));
	...
	/**
	 * Função do botão AdicionarVenda responsável por abrir a tela para adição
	 * de Venda
	 * 
	 * @param Event
	 * @throws IOException
	 */
	@FXML
	public void onBtAdicionarVenda(ActionEvent Event) throws IOException {

		Parent root = FXMLLoader.load(getClass().getResource("/views/AdicionarVendaAbertoView.fxml"));
		Stage primaryStage = new Stage();
		Scene scene = new Scene(root);
		primaryStage.setTitle("Cadastro de Venda");
		primaryStage.setScene(scene);
		primaryStage.initModality(Modality.APPLICATION_MODAL);
		primaryStage.initOwner(btNovaVenda.getScene().getWindow());
		primaryStage.show();
	...
	/**
	 * Função do botão EditarVenda responsável por abrir a tela para edição de
	 * Venda
	 * 
	 * @param Event
	 * @throws IOException
	 */
	@FXML
	public void onBtEditarVenda(ActionEvent Event) throws IOException {

		Parent root = FXMLLoader.load(getClass().getResource("/views/EditarVendaAbertoView.fxml"));
		Stage primaryStage = new Stage();
		Scene scene = new Scene(root);
		primaryStage.setTitle("Edição de Venda");
		primaryStage.setScene(scene);
		primaryStage.initModality(Modality.APPLICATION_MODAL);
	 ...
	 */
	@FXML 
	public void onBtRealizar(ActionEvent Event) throws IOException{
		
		Parent root = FXMLLoader.load(getClass().getResource("/views/RealizarVendaView.fxml"));
		Stage primaryStage = new Stage();
		Scene scene = new Scene(root);
		primaryStage.setTitle("Realização de Venda");
		primaryStage.setScene(scene);
		primaryStage.initModality(Modality.APPLICATION_MODAL);
		primaryStage.initOwner(btRealizar.getScene().getWindow());
		primaryStage.show();
	...
	public void checarIdBuscaDetalhe() {
		if (Verificacoes.verificaConversaoNumeroInt(idBuscaDetalhe.getText())) {
			...
			if (Verificacoes.verificaExistenciaIdVenda(id, GerenciarVenda.getVendasEmAberto())) {

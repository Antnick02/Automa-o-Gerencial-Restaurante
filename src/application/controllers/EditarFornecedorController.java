...
public class EditarFornecedorController implements Initializable {
	...
	private void checarEntradas() {
		...
		String entradaId = idFornecedor.getText();

		if (Verificacoes.verificaIdFornecedor(entradaId)) {
			
			int idFornecedor1 = Integer.parseInt(entradaId);
			...
			if ((entradaNome != "") && (entradaCnpj != "") && (entradaEndereco != "")) {
				...
				if (Buscas.buscaNomeFornecedor(entradaNome)) {
					...
					if (Verificacoes.verificaConversaoNumeroInt(entradaCnpj)) {
						...
						fornecedor.setIdFornecedor(idFornecedor1);
						GerenciarFornecedor.editarFornecedor(fornecedor);
				...
				Alertas.showAlert(null, "Preencha todos os campos para editar um fornecedor", null, AlertType.WARNING);
			}

		}else {
			Alertas.showAlert(null, "Id inválido!!",
					"È inexistente ou inválido ", AlertType.WARNING);
		}
	}

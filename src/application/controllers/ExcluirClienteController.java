	...
	private void checarEntrada() {
		...
		if (Verificacoes.verificaExclusaoIdCliente(entradaIdCliente, GerenciarVenda.getVendasEmAberto())) {
			if (Verificacoes.verificaExclusaoIdCliente(entradaIdCliente, GerenciarVenda.getVendas())) {
				...
				Alertas.showAlert(null, "Este cliente ainda está vinculado a alguma venda realizada!!", null,
						AlertType.WARNING);
			}
		} else {
			Alertas.showAlert(null, "Id Inválido, inexistente ou este cliente está vinculado a alguma venda em aberto", null, AlertType.WARNING);
		}
	}
